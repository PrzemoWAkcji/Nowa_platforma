...
async function testImportWithCookies() {
  try {
    ...
    // 1. Zaloguj się i pobierz cookie
    ...
    // Pobierz cookie z nagłówka Set-Cookie
    const setCookieHeader = loginResponse.headers.get('set-cookie');
    if (setCookieHeader) {
      cookies = setCookieHeader.split(';')[0]; // Pobierz tylko część z tokenem
    }
    ...
    const importResponse = await fetch(`${API_BASE}/competitions/${competition.id}/import-startlist-json`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': cookies,
    ...
    if (eventsResponse.ok) {
      ...
      const relayEvents = events.filter(event => event.name.includes('4x'));
