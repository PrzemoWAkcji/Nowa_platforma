...
export class LiveResultsService {
  ...
  /**
   * Pobiera wyniki na żywo dla zawodów
   */
  async getLiveResults(token: string) {
    ...
    // Formatuj dane dla wyświetlenia
  ...
  /**
   * Pobiera specyfikacje sprzętu dla konkurencji
   */
  private getEquipmentSpecsForEvent(event: any): string[] {
    const specs: string[] = [];
    ...
    if (event.implementSpecs) {
      ...
      if (additionalSpecs.discus) specs.push(`Dysk: ${additionalSpecs.discus}`);
      if (additionalSpecs.hammer) specs.push(`Młot: ${additionalSpecs.hammer}`);
