/**
 * Skrypt migracji danych ze SQLite do PostgreSQL (Supabase)
 * Używa bezpośredniego połączenia z SQLite poprzez better-sqlite3
 */
...
// Ścieżka do starej bazy SQLite
const sqlitePath = path.join(__dirname, 'prisma', 'dev.db');

// Klient dla PostgreSQL (używa DATABASE_URL z .env)
...
// Funkcja konwertująca wartości SQLite na odpowiednie typy dla PostgreSQL
...
async function migrateData() {
  ...
  try {
    ...
    // 2. Migracja zawodników
    console.log('🏃 Migracja zawodników...');
    const athletes = db.prepare('SELECT * FROM athletes').all();
    for (const athlete of athletes) {
      // Oblicz kategorię na podstawie daty urodzenia lub ustaw domyślnie SENIOR
      let category = 'SENIOR';
      if (athlete.dateOfBirth) {
        ...
        else if (age >= 55 && age < 60) category = 'M55';
        else if (age >= 60 && age < 65) category = 'M60';
        else if (age >= 65) category = 'M65';
      }
    ...
    // Pobierz pierwszego użytkownika jako domyślnego twórcę
    ...
    // 4. Migracja wielobojów (jeśli istnieją)
    ...
    for (const event of events) {
      // Mapuj ageCategory na category lub użyj domyślnego
      let category = event.ageCategory || 'SENIOR';

      // Określ unit na podstawie typu wydarzenia i nazwy
      let unit = 'TIME';
      if (event.type === 'FIELD') {
        // Skoki = HEIGHT, Rzuty = DISTANCE
        if (
          event.name &&
    ...
    for (const result of results) {
      // Mapuj stare pole status na nowe pola boolean
    ...
    // 8. Migracja rekordów (jeśli istnieją)
